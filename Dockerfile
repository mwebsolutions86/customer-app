# Dockerfile pour Customer App (React Native/Expo)
FROM node:18-alpine
...
COPY customer-app/package.json customer-app/package-lock.json* ./
...
RUN echo '#!/bin/sh' > /app/build.sh && \
    echo 'echo "🏗️ Building Customer App..."' >> /app/build.sh && \
...
RUN echo '#!/bin/sh' > /app/test.sh && \
    echo 'echo "🧪 Testing Customer App..."' >> /app/test.sh && \
